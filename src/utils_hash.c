...
struct hash_context_tag {
    HashConfig_t        config;

    ThreadMutexHandle_t *bucket_mutex;
    struct hlist_head   *bucket_head;
};
typedef struct hash_context_tag hash_context_t;
...
static hal_int32_t _key_to_index(hal_int32_t bucket_max_len, 
                                 const hal_char_t* key)
{
    if (NULL == key) {
...
static hal_int32_t _find_item_from_list(hash_context_t   *context, 
                                        ...
                                        hal_uint32_t     index)
{
    ...
    hal_uint32_t key_hash = 0; 

    key_hash = UtilsHashGenerate(hash_item->key);
...
hal_int32_t UtilsHashItemAdd(HashHandle_t handle, HashItem_t *hash_item)
{
    ...
    index = _key_to_index(context->config.bucket_max_len, hash_item->key);
    find_flag = _find_item_from_list(context, hash_item, _replace_item_val, index);
...
hal_int32_t UtilsHashItemDel(HashHandle_t handle, HashItem_t *hash_item)
{
    ...
    index = _key_to_index(context->config.bucket_max_len, hash_item->key);
    return _find_item_from_list(context, hash_item, _del_item_from_list, index);
}
...
hal_int32_t UtilsHashItemGet(HashHandle_t handle, HashItem_t *hash_item)
{
    ...
    index = _key_to_index(context->config.bucket_max_len, hash_item->key);
    return _find_item_from_list(context, hash_item, _get_item_val, index);
}
